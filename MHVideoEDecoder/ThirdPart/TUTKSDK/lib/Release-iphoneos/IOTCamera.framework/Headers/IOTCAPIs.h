...
------------|------------------|---------------|-------------------
3.1.10.5    |Terry Liu         |2018-05-20     |o Mark IOTC_Device_LoginNB, IOTC_Get_Device_Status, IOTC_Accept as deprecated
3.1.10.5    |Terry Liu         |2018-05-20     |o Mark IOTC_Connect_ByUIDNB, IOTC_Connect_ByUID_ParallelNB, IOTC_Session_Write_Reliable_NB as deprecated
3.1.10.5    |Terry Liu         |2018-05-20     |o Mark IOTC_Session_Write_Reliable_NB_Abort, IOTC_Reliable_All_MSG_Is_Sent, IOTC_Initialize as deprecated
3.1.10.5    |Terry Liu         |2018-05-20     |o Mark IOTC_Setup_DetectNetwork_Timeout, IOTC_Setup_ErrorUpload, IOTC_ConnModeChange_CallBack as deprecated
3.1.10.5    |Terry Liu         |2018-05-20     |o Mark IOTC_Lan_Search, IOTC_Lan_Search2 as deprecated
 */
...
typedef enum
{
	AUTHENTICATE_BY_KEY                  //Create IOTC Session with authentication by Key
...
//For IOTC_Device_LoginEx, IOTC_Connect_byUIDEx
typedef struct DeviceLoginInput {
	unsigned int cb;							//Check byte for structure size
...
typedef struct IOTCConnectInput {
	...
	IOTCAuthenticationType authentication_type;
	char auth_key[IOTC_AUTH_KEY_LENGTH]; 			//allow '0'~'9' & 'A'~'Z' & 'a'~'z'
	unsigned int timeout;  						// set 0 as default behavior
}IOTCConnectInput;
 ...
 */
P2PAPI_API_DEPRECATED int IOTC_Initialize(unsigned short nUDPPort, const char* cszP2PHostNamePrimary,
 ...
 *			- #IOTC_ER_RESOURCE_ERROR Getting system resource fail. (i.e. socket, thread etc.)
 *
 * \attention (1) This API has been deprecated and might be removed in the next version, please replace it with IOTC_device_Login
 *
 */
P2PAPI_API_DEPRECATED int IOTC_Device_LoginNB(const char *cszUID, const char *cszDeviceName, const char *cszDevicePWD,
 ...
 * 				into server successfully.
 */
P2PAPI_API int IOTC_Device_LoginEx(const char *cszUID, const DeviceLoginInput *optionalInput);
/**
 ...
 *
 * \attention (1) This API has been deprecated and might be removed in the next version, please replace it with IOTC_Listen
 *
 */

P2PAPI_API_DEPRECATED int IOTC_Get_Device_Status(struct st_DeviceStInfo * pDevStInfo);
...
P2PAPI_API_DEPRECATED int  IOTC_Accept(int *SID);
 ...
 *
 * \attention This API is a blocking function. This function will wait until
 ...
 *			- #IOTC_ER_NOT_SUPPORT_RELAY Not support relay connection by IOTC servers
 *
 * \attention (1) This API has been deprecated and might be removed in the next version, please replace it with 
 *                IOTC_Connect_ByUID or IOTC_Connect_ByUID_Parallel
 *
 */
P2PAPI_API_DEPRECATED int  IOTC_Connect_ByUIDNB(const char *cszUID,
 ...
 * \param connectInput [in] A pointer that points to a memory which the login 
 *						   information is saved to.
 *
 * \return IOTC session ID if return value >= 0
 * \return Error code if return value < 0
 ...
 *
 */
P2PAPI_API int IOTC_Connect_ByUIDEx(const char *cszUID, int SID, IOTCConnectInput *connectInput );
/**
 ...
 *			- #IOTC_ER_INVALID_SID The specified IOTC session ID is not valid
 *
 * \attention (1) If you call IOTC_Connect_Stop_BySID() and this function not return yet, and then use the same
 *              session ID to call again will cause session ID in wrong status.
 *            (2) This API has been deprecated and might be removed in the next version, please replace it with IOTC_Connect_ByUID_Parallel
 *
 */
P2PAPI_API_DEPRECATED int  IOTC_Connect_ByUID_ParallelNB(const char *cszUID, int SID,
											  connectStateHandler handler,
											  void * userData);
 ...
 */
P2PAPI_API_DEPRECATED int IOTC_Lan_Search(struct st_LanSearchInfo *psLanSearchInfo, int nArrayLen, int nWaitTimeMs);
 ...
 *			- IOTC_ER_INVALID_ARG   Invalid input argument.
 */
P2PAPI_API int IOTC_Set_Log_Attr(LogAttr logAttr);
 ...
 * 			  (2) This API has been deprecated. Please use IOTC_Search_Device_Start / IOTC_Search_Device_Result.<br><br>*
 */
P2PAPI_API_DEPRECATED int IOTC_Lan_Search2(struct st_LanSearchInfo2 *psLanSearchInfo2, int nArrayLen, int nWaitTimeMs);
 ...
 *
 * \attention (1) Must be called before start login or connection.
 *            (2) This API has been deprecated and might be removed in the next version, there is no replacement for this API
 *
 */
P2PAPI_API_DEPRECATED void IOTC_Setup_DetectNetwork_Timeout(unsigned int nTimeout);
...
*				to get more IOTC channel IDs and then specifying those IOTC channel IDs
*				in this function according to the purpose defined by the user. <br><br>
*            (2) IOTC reliabe non-blocking call has a send queue. Before calling IOTC_Session_Close()
...
*            (3) This API has been deprecated and might be removed in the next version, please replace it with RDT
*
*/
P2PAPI_API_DEPRECATED int IOTC_Session_Write_Reliable_NB(int nIOTCSessionID, const char *cabBuf, int nBufSize, unsigned char nIOTCChannelID);
...
*				 to get more IOTC channel IDs and then specifying those IOTC channel IDs
*				 in this function according to the purpose defined by the user. <br><br>
*            (2) This API has been deprecated and might be removed in the next version, please replace it with RDT
*
**/
P2PAPI_API_DEPRECATED int IOTC_Session_Write_Reliable_NB_Abort(int nIOTCSessionID, unsigned char nIOTCChannelID);
...
*			- #IOTC_ER_CH_NOT_ON The IOTC channel of specified channel ID is not turned on
*
* \attention (1) This API has been deprecated and might be removed in the next version, please replace it with RDT
*
**/
P2PAPI_API_DEPRECATED int IOTC_Reliable_All_MSG_Is_Sent(int nIOTCSessionID, unsigned char nIOTCChannelID);
...
*
* \attention (1) This API has been deprecated and might be removed in the next version, there is no replacement for this API
*
**/
P2PAPI_API_DEPRECATED void IOTC_Setup_ErrorUpload(int nEnable);
...
*
* \attention (1) This API has been deprecated and might be removed in the next version, please use IOTC_Session_Check to replace this API
*
**/
P2PAPI_API_DEPRECATED void IOTC_ConnModeChange_CallBack(ConnectModeChangeCB fpCallback);
...
**/
P2PAPI_API int IOTC_Device_Update_Authkey(const char *new_authkey);
...
*           - #IOTC_ER_ALREADY_INITIALIZED IOTC module is already initialized
**/
P2PAPI_API int IOTC_Set_Master_Region(Region region);

/**
* \brief Initialize DebugTool of IOTC module.
*
* \details Enable device to collect system information and report to client in LAN mode.
*
* \param pwd [in] User must input 8 bytes ASCII password.
*
* \attention (1) This function shall be called before IOTC platform is initialized.
*            (2) Client can call IOTC_Get_DebugTool_Info() to get device report on in LAN mode.
*
* \return Return IOTC_ER_NoERROR if successfully.
* \return Error code if return value < 0
*			- #IOTC_ER_INVALID_ARG if password length is not 8 byte, not ASCII character or NULL pointer.
*			- #IOTC_ER_ALREADY_INITIALIZED Call this function after IOTC module initialize.
**/
P2PAPI_API int IOTC_DebugTool_Initialize(char *pwd);

/**
* \brief Client can collect sdk information with this api
*
* \details Client can connect to device which DebugTool is turn on and collect sdk information in LAN mode.
*
* \param uid [in] Device's UID
* \param pwd [in] Device's debugtool password(8 bytes ASCII)
* \param buf [out] The array of byte buffer to receive system information of JSON.
* \param buf_size [in] The length of the byte buffer.
* \param data_size [out] The actual length of system information in byte.
* \param timeout_sec [in] The timeout for this function in unit of second, give 0 means default value(10 second).
*
* \return Return IOTC_ER_NoERROR if successfully.
* \return Error code if return value < 0
*			- #IOTC_ER_INVALID_ARG if UID or password length invalid, not ASCII character or NULL pointer.
*			- #IOTC_ER_NOT_ENOUGH_MEMORY if buf_size less than data_size.
*			- #IOTC_ER_FAIL_CREATE_SOCKET if create TCP socket fail.
*			- #IOTC_ER_TCP_CONNECT_TO_SERVER_FAILED if TCP socket connect fail.
*			- #IOTC_ER_SERVER_NOT_RESPONSE if TCP socket receive data fail after connect success.
*			- #IOTC_ER_TIMEOUT Return this code when device is offline, device not call IOTC_DebugTool_Initialize(),
*				wrong UID, wrong password.
*
* \attention (1) This API is a blocking function.
*
**/

P2PAPI_API int IOTC_Get_DebugTool_Info(const char *uid, const char *pwd, char *buf, int buf_size, int *data_size, int timeout_sec);
