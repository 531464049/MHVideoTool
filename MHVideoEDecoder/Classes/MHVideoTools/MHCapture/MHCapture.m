...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection{
    if (self.isCapturing) {
        ...
        if ([self.videoDataOutput isEqual:captureOutput]) {
            [self sendVideoSample:pixelBuffer];
            
        }else if([self.audioDataOutput isEqual:captureOutput]){
            [self sendAudioSample:pixelBuffer];
        }
    }
}
...
-(void)sendVideoSample:(CVImageBufferRef)sampleBuffer
{
    //NSLog(@"retainCount = %lu",CFGetRetainCount(sampleBuffer));
    //不管是GPUImage返回的buffer还是系统摄像头返回的buffer，retainCount都是1，但是系统摄像头返回的buffer不需要release！！why？？
    //莫非是captureOutput: didOutputSampleBuffer: fromConnection:之后会自动release
    if (self.delegate && [self.delegate respondsToSelector:@selector(mhCaptureSendVideoSampleBuffer:)]) {
        [self.delegate mhCaptureSendVideoSampleBuffer:sampleBuffer];
    }else{
        if (self.captureType == MHAVCaptureTypeGPUImage) {
            CFRelease(sampleBuffer);
        }
    }
}
-(void)sendAudioSample:(CVImageBufferRef)sampleBuffer
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(mhCaptureSendAudioSampleBuffer:)]) {
        [self.delegate mhCaptureSendAudioSampleBuffer:sampleBuffer];
    }else{
        if (self.captureType == MHAVCaptureTypeGPUImage) {
            CFRelease(sampleBuffer);
        }
