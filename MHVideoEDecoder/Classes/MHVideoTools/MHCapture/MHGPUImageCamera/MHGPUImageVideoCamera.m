...
-(void)processAudioSample:(CMSampleBufferRef)sampleBuffer{
    ...
    if (self.mhGPUOutDelegate && [self.mhGPUOutDelegate respondsToSelector:@selector(sendAudioSample:)]) {
        CVImageBufferRef pixelBuffer = (CVImageBufferRef)CMSampleBufferGetImageBuffer(sampleBuffer);
        [self.mhGPUOutDelegate sendAudioSample:pixelBuffer];
    }else{
...
-(void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex{
    ...
    if (self.mhGPUOutDelegate && [self.mhGPUOutDelegate respondsToSelector:@selector(sendVideoSample:)]) {
        //32BGRA转Y_UV
        pixelBuffer = [MHCVPixelBuffer convertRGB_BuffertoYUV:pixelBuffer];
        [self.mhGPUOutDelegate sendVideoSample:pixelBuffer];
    }else{
